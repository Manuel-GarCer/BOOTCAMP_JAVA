        ...
        try {
            if(validateSignUp(requestMap)){
                ...
                if(Objects.isNull(usuarios)){
                    ...
                    return MarketUtils.getResponseEntity("Usuario Registrado con éxito", HttpStatus.CREATED);
                }else{
                    return MarketUtils.getResponseEntity("Usuario Ya existe", HttpStatus.BAD_REQUEST);
                }
            }else {
                return MarketUtils.getResponseEntity(Constantes.DATA_INVALIDA, HttpStatus.BAD_REQUEST);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
        return MarketUtils.getResponseEntity(Constantes.ALGO_SALIO_MAL,HttpStatus.INTERNAL_SERVER_ERROR);
    }
